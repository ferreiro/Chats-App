...
void loadChat(ifstream &file, string username, tChat &chat, bool &ok) {
	string auxName;
...
void openSingleChat(tChat &chat, tMessageList &messageList) {
	...
	do {
		...
		else if (chat.messages.counter == MAX_MESSAGES) 
		{
			// No more space Avaliable
			setColor(light_red);
			cout << endl << setw(PADDING) << left << " " << "You can't send one more message. Memory Full" << endl;
			cout << setw(PADDING) << left << " " << "Maximun of Messages per Chat: " << MAX_MESSAGES << endl;
			Sleep(2000);
			setColor(white);
		}
		else if ((inputMessage != "$quit$") && (chat.messages.counter < MAX_MESSAGES)) 
		{
			// Insert a Message
